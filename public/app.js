...
const defaultProfile = {
  ...
  shortDescription: 'منصة تقييم احترافية لجمع رأي العملاء حول جودة البضاعة والخدمة.',
...
const elements = {
  ...
  companyNameAside: document.getElementById('companyNameAside'),
  heroDescription: document.getElementById('heroDescription'),
  companyDescriptionAside: document.getElementById('companyDescriptionAside'),
  ...
  emailPrimary: document.getElementById('emailPrimary'),
  addressPrimary: document.getElementById('addressPrimary'),
...
function listenToAuth() {
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user && user.uid !== ADMIN_UID) {
      showStatus(elements.adminLoginStatus, 'تم تسجيل الدخول لكن هذا الحساب ليس الأدمن المحدد.', 'error');
      await signOut(auth);
    }
  });
}
...
async function handleAdminLogin(event) {
  ...
  try {
    showStatus(elements.adminLoginStatus, 'جاري التحقق من الحساب...', 'info');
    const credential = await signInWithEmailAndPassword(auth, email, password);

    if (credential.user.uid !== ADMIN_UID) {
      await signOut(auth);
      showStatus(elements.adminLoginStatus, 'هذا المستخدم لا يطابق UID الأدمن المسموح.', 'error');
      return;
    }

    showStatus(elements.adminLoginStatus, 'تم الدخول بنجاح.', 'success');
  } catch (error) {
...
async function loadCompanyProfile(fillAdminForm = false) {
  try {
    const companyDocRef = doc(db, ...COMPANY_DOC_PATH);
...
function renderPublicProfile(profile) {
  if (elements.heroCompanyName) elements.heroCompanyName.textContent = profile.companyName;
  if (elements.companyNameAside) elements.companyNameAside.textContent = profile.companyName;
  if (elements.heroDescription) elements.heroDescription.textContent = profile.shortDescription || '';
  if (elements.companyDescriptionAside) elements.companyDescriptionAside.textContent = profile.shortDescription || '';
  if (elements.locationText) elements.locationText.textContent = profile.address || 'سيتم تحديث العنوان قريبًا';
  if (elements.workingHoursText) elements.workingHoursText.textContent = profile.workingHours || 'ساعات العمل غير محددة';
  if (elements.phonePrimary) elements.phonePrimary.textContent = profile.phones[0] || '—';
  if (elements.emailPrimary) elements.emailPrimary.textContent = profile.email || '—';
  if (elements.addressPrimary) elements.addressPrimary.textContent = profile.address || '—';
  if (elements.heroLogo) elements.heroLogo.src = profile.logoUrl || './assets/logo-placeholder.svg';
  ...
  if (profile.websiteUrl) {
    ...
    elements.websiteLinkTop.setAttribute('aria-label', 'الموقع الرسمي');
  } else {
    elements.websiteLinkTop.href = '#surveySection';
...
function renderContacts(profile) {
  const items = [
    { label: 'الهاتف', value: profile.phones[0] || '—', icon: phoneIcon() },
    { label: 'البريد', value: profile.email || '—', icon: mailIcon() },
    { label: 'واتساب', value: profile.whatsappNumber || '—', icon: whatsappIcon() },
    { label: 'العنوان', value: profile.address || '—', icon: mapIcon() },
  ];

  elements.contactGrid.innerHTML = items
    .map(
      (item) => `
...
function renderSocialLinks(profile) {
  const socialLinks = [
    ...
    { href: profile.websiteUrl, name: 'Website', icon: globeIcon() },
  ].filter((item) => Boolean(item.href));

  elements.socialStrip.innerHTML = socialLinks.length
    ? socialLinks
        .map(
          (item) => `
      ...
      `,
        ...
        .join('')
    : '<div class="muted">أضف روابط التواصل الاجتماعي من لوحة الإدارة لتظهر الأيقونات هنا.</div>';
}
...
async function saveCompanyProfile(event) {
  ...
  if (!user || user.uid !== ADMIN_UID) {
    showStatus(elements.settingsStatus, 'غير مصرح لك بحفظ البيانات.', 'error');
    return;
  }
  ...
  try {
    showStatus(elements.settingsStatus, 'جاري حفظ الإعدادات...', 'info');
    await setDoc(doc(db, ...COMPANY_DOC_PATH), profile, { merge: true });
    activeProfile = normalizeProfile(profile);
    renderPublicProfile(activeProfile);
    renderResponsesTable();
    showStatus(elements.settingsStatus, 'تم حفظ بيانات الشركة بنجاح.', 'success');
  } catch (error) {
...
async function handleSurveySubmit(event) {
  ...
  const serviceRating = Number(document.getElementById('serviceRating').value);
  if (!productRating || !serviceRating) {
    showStatus(elements.surveyStatus, 'يرجى اختيار تقييم البضاعة والخدمة.', 'error');
    return;
  }
  ...
  try {
    ...
    showStatus(elements.surveyStatus, 'جاري إرسال التقييم...', 'info');
    ...
    showStatus(elements.surveyStatus, 'تم إرسال تقييمك بنجاح.', 'success');
...
async function loadResponses() {
  try {
    ...
    allResponses = snapshot.docs.map((docItem) => ({ id: docItem.id, ...docItem.data() }));
...
function renderDashboardStats() {
  ...
  const latest = total ? allResponses[0].createdAtText : '—';
  ...
  elements.statUpdated.textContent = latest;
}

function renderSeasonAverages() {
  const seasons = groupBySeason(allResponses);
  const keys = Object.keys(seasons);
  if (!keys.length) {
    elements.seasonAverages.innerHTML = '<div class="muted">لا توجد بيانات لعرض متوسطات المواسم بعد.</div>';
    return;
  }

  elements.seasonAverages.innerHTML = keys
    .map((season) => {
...
function renderResponsesTable() {
  ...
  if (!filtered.length) {
    elements.responsesTableBody.innerHTML = '<tr><td colspan="7" class="empty-state">لا توجد ردود مطابقة للفلاتر الحالية.</td></tr>';
    return;
  }
...
function humanizeFirebaseError(error) {
  const map = {
    ...
    'storage/unauthorized': 'ليس لديك صلاحية للوصول إلى التخزين.',
  };
...
function mailIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="5" width="18" height="14" rx="2"></rect><path d="m3 7 9 6 9-6"></path></svg>';
}
function mapIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 21s-6-5.33-6-11a6 6 0 0 1 12 0c0 5.67-6 11-6 11z"></path><circle cx="12" cy="10" r="2.5"></circle></svg>';
}
function whatsappIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M20.52 3.48A11.86 11.86 0 0 0 12.02 0C5.4 0 .02 5.38.02 12c0 2.11.55 4.16 1.6 5.97L0 24l6.2-1.62A11.98 11.98 0 0 0 12.02 24h.01c6.61 0 11.99-5.38 11.99-12 0-3.2-1.25-6.2-3.5-8.52zM12.03 21.8a9.8 9.8 0 0 1-5-1.37l-.36-.22-3.68.96.98-3.58-.24-.37a9.8 9.8 0 1 1 8.3 4.58zm5.36-7.35c-.29-.14-1.72-.85-1.99-.95-.27-.1-.46-.14-.66.14-.19.29-.76.95-.93 1.15-.17.19-.34.22-.63.07-.29-.14-1.23-.45-2.34-1.45-.86-.77-1.44-1.72-1.61-2.01-.17-.29-.02-.44.13-.58.13-.13.29-.34.43-.51.14-.17.19-.29.29-.48.1-.19.05-.36-.03-.5-.07-.14-.66-1.59-.91-2.17-.24-.58-.48-.5-.66-.5h-.56c-.19 0-.5.07-.77.36-.26.29-1.01.99-1.01 2.42s1.03 2.81 1.18 3c.14.19 2.01 3.07 4.87 4.3.68.29 1.22.47 1.64.6.69.22 1.32.19 1.82.12.55-.08 1.72-.7 1.96-1.38.24-.68.24-1.27.17-1.39-.08-.12-.27-.19-.56-.34z"></path></svg>';
}
function facebookIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M13.5 22v-8h2.7l.4-3h-3.1V9.1c0-.9.3-1.6 1.6-1.6H17V4.8c-.8-.1-1.6-.2-2.4-.2-2.4 0-4.1 1.5-4.1 4.2V11H8v3h2.5v8h3z"></path></svg>';
}
function instagramIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="18" height="18" rx="5"></rect><circle cx="12" cy="12" r="4"></circle><circle cx="17.5" cy="6.5" r="1"></circle></svg>';
}
function xIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M18.9 2H22l-6.8 7.77L23 22h-6.2l-4.86-6.35L6.36 22H3.24l7.28-8.32L1 2h6.35l4.4 5.8L18.9 2zm-1.08 18h1.72L6.4 3.9H4.55L17.82 20z"></path></svg>';
}
function tiktokIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M14 3h3.02A5.98 5.98 0 0 0 21 7.98V11a8.9 8.9 0 0 1-4-1v6.45A5.45 5.45 0 1 1 11.55 11a5.4 5.4 0 0 1 1.45.2v3.18a2.3 2.3 0 1 0 1 1.9V3z"></path></svg>';
}
function snapchatIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 2c3 0 5 2 5 5v3.1c0 .6.3 1.2.8 1.5.4.2.8.3 1.2.4.4.1.6.5.5.9-.2.5-.8.8-1.6 1-.5.1-.8.5-.9 1-.1.6-.4 1.3-1.2 1.5-.6.2-1.2.3-1.8.3-.3.9-.9 1.3-2 1.3s-1.7-.4-2-1.3c-.6 0-1.2-.1-1.8-.3-.8-.2-1.1-.9-1.2-1.5-.1-.5-.4-.9-.9-1-.8-.2-1.4-.5-1.6-1-.1-.4.1-.8.5-.9.4-.1.8-.2 1.2-.4.5-.3.8-.9.8-1.5V7c0-3 2-5 5-5z"></path></svg>';
}
function linkedinIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M6.94 8.5H3.56V20h3.38V8.5zM5.25 3A1.97 1.97 0 1 0 5.3 6.94 1.97 1.97 0 0 0 5.25 3zM20.44 13.11c0-3.45-1.84-5.05-4.3-5.05-1.98 0-2.86 1.09-3.36 1.86V8.5H9.4c.04.94 0 11.5 0 11.5h3.38v-6.42c0-.34.02-.68.13-.92.27-.68.88-1.38 1.91-1.38 1.35 0 1.89 1.03 1.89 2.54V20h3.38v-6.89z"></path></svg>';
}
function globeIcon() {
  return '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"></circle><path d="M3 12h18"></path><path d="M12 3a14 14 0 0 1 4 9 14 14 0 0 1-4 9 14 14 0 0 1-4-9 14 14 0 0 1 4-9z"></path></svg>';
}
